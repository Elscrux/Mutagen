...
namespace Mutagen.Bethesda.Generation.Modules.Plugin
{
    public class TriggeringRecordModule : GenerationModule
    {
        ...
        public override async Task FinalizeGeneration(ProtocolGeneration proto)
        {
            ...
            if (proto.DefaultNamespace.Equals("Mutagen.Bethesda.Plugins.Records")) return;
            HashSet<RecordType> recordTypes = new HashSet<RecordType>();
        ...
        public override async Task FinalizeGeneration(IEnumerable<ProtocolGeneration> protos)
        {
            if (!Program.ShouldRun("All")) return;

            var core = protos.FirstOrDefault(x => x.DefaultNamespace == "Mutagen.Bethesda.Plugins.Records");
            if (core == null) return;
            
            var recordTypes = new HashSet<RecordType>();
            ...
            using (var n = new NamespaceWrapper(fg, $"{core.DefaultNamespace}.Internals"))
            {
                using (var c = new ClassWrapper(fg, "RecordTypes"))
                {
                    c.Partial = true;
            ...
            var path = Path.Combine(core.DefFileLocation.FullName, $"RecordTypes{Loqui.Generation.Constants.AutogeneratedMarkerString}.cs");
            fg.Generate(path);
            core.GeneratedFiles.Add(path, ProjItemType.Compile);
            fg = new FileGeneration();
            using (var n = new NamespaceWrapper(fg, $"{core.DefaultNamespace}.Internals"))
            {
                using (var c = new ClassWrapper(fg, "RecordTypeInts"))
            ...
            path = Path.Combine(core.DefFileLocation.FullName, $"RecordTypeInts{Loqui.Generation.Constants.AutogeneratedMarkerString}.cs");
            fg.Generate(path);
            core.GeneratedFiles.Add(path, ProjItemType.Compile);
        }
    }
